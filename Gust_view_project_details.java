...
public class Gust_view_project_details extends AppCompatActivity implements View.OnClickListener {
    TextView title1;
    TextView report1;
    TextView abst1;
    TextView journal1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_gust_view_project_details);
                ...
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        ...
                        try {
                            ...
                            Toast.makeText(Gust_view_project_details.this, ""+response, Toast.LENGTH_SHORT).show();
                            String status=jsonObj.getString("status");

                            if (status.equalsIgnoreCase("ok")) {
                                String title= jsonObj.getString("title");
                                String abst= jsonObj.getString("abst");
                                String report=jsonObj.getString("report");
                                String demo= jsonObj.getString("demo");
                                String journal= jsonObj.getString("journal");
                                String githublink= jsonObj.getString("githublink");


                                Toast.makeText(Gust_view_project_details.this, "aaa", Toast.LENGTH_SHORT).show();
...
//                                startActivity(i);
    ...
    public void onClick(View view) {
        if(view==githublink1)
        {
            ...
            String url="http://" + ip + ":5000"+report1.getText().toString();
            Log.d("url------------",url);
            Log.d("---***","..**..%%..");
        ...
        if (view == download3) {
            SharedPreferences sh= PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
            String ip=sh.getString("ip","");
            String url="http://" + ip + ":5000"+journal1.getText().toString();
            Log.d("url------------",url);
            Log.d("---***","..**..%%..");
        ...
        if (view == download4) {
            SharedPreferences sh= PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
            String ip=sh.getString("ip","");
            String url="http://" + ip + ":5000"+demo1.getText().toString();
            Log.d("url------------",url);
            Log.d("---***","..**..%%..");
