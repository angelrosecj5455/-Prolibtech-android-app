...
staticpath = "C:\\Users\\Angel Rose\\PycharmProjects\\project_management\\static\\"
gl_list=[".","-","(",")","_","'",":",",","[","]","@","%","?","/","!","“",":",";","``","a","b","c","d","e","f","g","h",
         ...
         "your","yours","yourself","yourselves","you","yond","yonder","yon","ye","yet","zillion","umpteen","usually","username","uponed","upons","uponing","upon","ups", "upping", "upped", "up", "unto", "until", "unless", "unlike", "unliker", "unlikest", "under",
                                                        " ","thrissur","gec","my","for","we","tv","if","fig","fir","one","two","three","four","five",'six',"seven","eight","nine","ten",     "underneath",
...
@app.route('/ind_post', methods=['post'])
def ind_post():
    ...
    for i2 in ss:
        if i2 != '':
            ...
            # w = "SELECT distinct 'key_words'.'pid' project.*,student.s_name,department.dept_name FROM project INNER JOIN student ON student.login_id=project.stud_id INNER JOIN department ON department.dept_id=student.dept_id  INNER JOIN `key_words`  ON `project`.`project_id`=`key_words`.`pid` WHERE `key_words`.`key` LIKE '%" + i2 + "%' OR( `project`.title LIKE '%" + i2 + "%'OR( `project`.project_area LIKE '%" + i2 + "%')) AND project.p_year BETWEEN " + yr1 + " and " + yr2 + "  ORDER BY `count` DESC"
            w = "SELECT distinct `key_words`.`pid`, project.*,student.s_name,college.c_name,project.p_year FROM project INNER JOIN student ON project.stud_id=student.login_id INNER JOIN college ON student.college_id=college.college_id INNER JOIN `key_words`  ON `project`.`project_id`=`key_words`.`pid` WHERE `key_words`.`key` LIKE '%" + i2 + "%' OR( `project`.title LIKE '%" + i2 + "%'OR( `project`.project_area LIKE '%" + i2 + "%')) AND project.p_year BETWEEN " + yr1 + " and " + yr2 + "  ORDER BY `count` DESC"
            res = db.select(w)
            print(w)
            ...
            #                      's_name': i['s_name'],
            #                      'c_name': i['c_name'], 'p_year': i['p_year'], 'status': 'yes'}
            #                 ls.append(a)
            #             else:
            #                 a = {'title': i['title'], 'abstract': i['abstract'], 'project_id': i['project_id'],
            #                      's_name': i['s_name'],
            #                      'c_name': i['c_name'], 'p_year': i['p_year'], 'status': 'no'}
            #                 ls.append(a)
    print(ls)
    return render_template('ind.html',data=res)
...
def login_post():
   ...
   if ss is not None :
       ...
       if ss ['user_type']=='admin':
            ...
            qry = db.select("SELECT * FROM college WHERE login_id='" + str(ss['login_id']) + "'")
            res4 = db.selectOne(qry)
            print(res4)
            # session["name"] = qry["c_name"]
...
def view_staff(dept_id):
    ...
    print(res)
    return render_template('admin/viewviewview_project.html', data=res)
...
def report_word_extraction(pid):
    ...
    if res is not None:
        ...
        pdfFileObj = open(
            'C:\\Users\\Angel Rose\\PycharmProjects\\project_management\\static\\report\\20220429-104419.pdf', 'rb')
...
# # @app.route('/viewviewview_project/<project_id>')
# # def viewviewview_project(project_id):
# #     db = Db()
# #     res = db.select("SELECT project.*,student.s_name,department.dept_name FROM project INNER JOIN student ON student.login_id=project.stud_id INNER JOIN department ON department.dept_id=student.dept_id where project.project_id='"+str(project_id)+"'")
# #     print(res)
# #     return render_template('admin/viewviewview_project.html', data=res)
#
# # ----------------------------COLLEGE----------------------------------------------------
...
# # @app.route('/college_editprofile')
# # def college_editprofile():
# #     db = Db()
# #     qry = "SELECT * FROM `college` WHERE `college_id` ='" +str(session['lid'])+ "'"
# #     res = db.selectOne(qry)
# #     return render_template('college/college_editprofile.html', data=res)
#
# # @app.route('/college_editprofile_post',methods=['post'])
# # def college_editprofile_post():
# #     id = request.form['h1']
# #     c_name = request.form['textfield']
# #     place = request.form['textfield2']
# #     post = request.form['textfield3']
...
# #             qry="UPDATE `college` SET c_name='"+c_name+"',place='"+place+"', post='"+post+"',district='"+district+"', c_state='"+state+"', pin='"+pin+"',phone='"+phone+"',email='"+email+"',website='"+website+"',image='"+path+"' WHERE college_id='"+str(id)+"'"
# #             res=db.update(qry)
# #             return '''<script>alert('Updated');window.location='/college_editprofile'</script>'''
# #         else:
# #             qry = "UPDATE `college` SET c_name='" + c_name + "',place='" + place + "', post='" + post + "',district='" + district + "', c_state='" + state + "', pin='" + pin + "',phone='" + phone + "',email='" + email + "',website='" + website + "' WHERE college_id='"+str(id)+"'"
# #             res = db.update(qry)
# #             return '''<script>alert('Updated');window.location='/college_editprofile'</script>'''
...
def add_department_post():
    db = Db()
    dept_name = request.form['textfield']
    db.insert("insert into department (college_id,dept_name) VALUES ('"+str(session['lid'])+"','" + dept_name +"')")
...
# # def view_student():
# #     db = Db()
# #     res = db.select("SELECT `student`.*,`department`.`dept_name`,`branch`.`branch_name` FROM `department` INNER JOIN `student` ON `student`.`dept_id`=`department`.`dept_id` INNER JOIN `branch` ON `student`.`branch_id`= `branch`.`branch_id` WHERE `student`.`college_id` ='"+str(session['lid'])+"'")
# #     print(res)
# #     re = db.select("SELECT * FROM `branch` WHERE `college_id` ='" + str(session['lid']) + "'")
# #     print(re)
# #     dept=db.select("SELECT * FROM `department` WHERE college_id = '" + str(session["lid"]) + "'")
# #     print(dept)
# #     return render_template('college/view_student.html',data=res,dept=dept,branch=re)
# #
...
# #
# #         db = Db()
# #         qry="SELECT `student`.*,`department`.`dept_name`,`branch`.`branch_name` FROM `student` INNER JOIN branch ON `student`.`branch_id`=`branch`.`branch_id` INNER JOIN `department` ON `branch`.`dept_id`=`department`.`dept_id` WHERE `student`.`college_id` ='"+str(session['lid'])+"'  and `department`.`dept_name` = '"+dept+"' and `branch`.`branch_name` = '"+branch+"' AND `student`.`year` = '"+year+"' "
# #         res = db.select(qry)
# #
# #     elif btn=="view":
# #         search = request.form['textbox10']
...
# #     print("------------------------------")
# #     print(qry)
# #     return render_template('college/view_student.html',data=res)
#
#
...
#     # dept = db.select("SELECT * FROM `department` WHERE college_id = '" + str(session["lid"]) + "'")
#     # print(dept)
#     # return render_template('college/view_projects.html',data=res)
# # @app.route('/select_dept')
# # def select_dept():
# #     db=Db()
# #     res = db.select("SELECT * FROM `department` WHERE `college_id` ='" + str(session['lid']) + "'")
# #     print(res)
# #     return render_template('college/view_projects.html', dept=res)
# # @app.route('/brnch_ajax',methods=["post"])
# # def brnch_ajax():
# #     depid=request.form['depid']
# #     db=Db()
# #     q="SELECT * FROM `branch` WHERE `dept_id`='"+depid+"'"
...
def staffedit_profile_post(image=None):
    id = request.form['h1']
    db = Db()
    if 'fileField1' in request.files:
        ...
        if image.filename!="":
            ...
            res=db.update(qry)
            return '''<script>alert('helooo');window.location='/staff_profile'</script>'''
#         else:
#             # dt = time.strftime("%Y%m%d-%H%M%S")
#             # image.save(staticpath + "college_img\\" + dt + ".jpg")
#             # path = "/static/staff_img/" + dt + ".jpg"
...
def view_projects1_post():
    ...
    if(btn=="Submit"):
        ...
        db = Db()
        # qry = "select student.*,department.dept_name,branch.branch_name FROM `department` INNER JOIN `student` ON `student`.`dept_id`=`department`.`dept_id` INNER JOIN `branch` ON `student`.`branch_id`= `branch`.`branch_id` WHERE student.`branch_id`='" + str(session['branch_id'])+"' and `student`.`s_name` LIKE '%" + search + "%'"
        res = db.select("SELECT project.*,student.s_name,department.dept_name,branch.branch_name FROM project INNER JOIN student ON project.stud_id=student.login_id INNER JOIN department ON student.dept_id=department.dept_id INNER JOIN `branch` ON student.branch_id=branch.branch_id WHERE student.branch_id='" + str(session['branch_id']) + "' AND `project`.`title` LIKE '%" + search + "%' and project.status!='rejected'")
...
#     # phone = request.form['textfield3']
    ...
    # print(p["s_name"])
        lid=db.insert("insert into login(username,password,user_type) VALUES('" + email + "','" + passw + "','student')")
        db.insert("insert into student (college_id,`s_name`,email,phone,dept_id,branch_id,`s_year`,login_id) VALUES ('"+str(session['lid'])+"','" + student_name + "','" + email + "','" + phoneno + "','" + department + "','" + branch + "','" + year + "','"+str(lid)+"')")
...
#___________________________________________________
    # # creating a pdf file object
    # pdfFileObj = open(
    #     "C:\\Users\\Angel Rose\\PycharmProjects\\project_management\\static\\report\\" + timestr_report + ".pdf", 'rb')
    #
    # # creating a pdf reader object
    # pdfReader = PyPDF2.PdfFileReader(pdfFileObj)
    #
    ...
    doc = aw.Document("C:\\Users\\Angel Rose\\PycharmProjects\\project_management\\static\\report\\" + timestr_report + ".pdf")

    # convert PDF to Word DOCX format
    doc.save("C:\\Users\\Angel Rose\\PycharmProjects\\project_management\\static\\convert_doc\\"+timestr_report + ".docx")
    import docx
    doc = docx.Document("C:\\Users\\Angel Rose\\PycharmProjects\\project_management\\static\\convert_doc\\"+timestr_report + ".docx")  # Creating word reader object.
    data = ""
    fullText = []
...
#     # for i in res:
#     #     if i != '':
#     #         Q = "SELECT distinct `key_words`.`pid`, project.project_id,project.title,student.s_name,college.c_name,project.p_year FROM project INNER JOIN student ON project.stud_id=student.login_id INNER JOIN college ON student.college_id=college.college_id INNER JOIN `key_words`  ON `project`.`project_id`=`key_words`.`pid` WHERE `key_words`.`key` LIKE '%" + i + "%'  OR( `project`.title LIKE '%" + i + "%')  ORDER BY `count` DESC"
#     #         ss = db.select(Q)
#     #         print(ss)
#     #         for i in ss:
#     #
...
#     #             else:
#     #                 pid.append(i["project_id"])
#     #                 a = {'title': i["title"], 's_name': i["s_name"], 'c_name': i["c_name"], 'p_year': i["p_year"],
...
#     for i2 in ss:
#         if i2!='':
#             w="SELECT distinct `key_words`.`pid`,key_words.count, project.project_id,project.title,`project`.`abstract`,student.s_name,college.c_name,project.p_year FROM project INNER JOIN student ON project.stud_id=student.login_id INNER JOIN college ON student.college_id=college.college_id INNER JOIN `key_words`  ON `project`.`project_id`=`key_words`.`pid` WHERE `key_words`.`key` LIKE '%" + i2 + "%' OR( `project`.title LIKE '%" + i2 + "%'OR( `project`.project_area LIKE '%"+i2+"%'))  ORDER BY key_words.`count` DESC"
#             res = db.select(w)
#             print(w)
#
#             if len(res) > 0:
